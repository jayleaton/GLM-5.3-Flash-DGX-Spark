...
def prepare_prompt(tokenizer, target, nonce, output_lines):
    task=(f'\nWrite {output_lines} numbered Python comment lines explaining practical '
          f'software debugging techniques. Each line must contain a distinct, concrete '
          f'tip. End after line {output_lines}. Do not summarize the archive.\n')
...
def main():
    ...
    ap.add_argument('--output-lines',type=int,default=40,
                    help='Natural-stop numbered-line workload size; use identically for matched runs')
    a=ap.parse_args()
    ...
    if min(lengths+cs+[a.repeats,a.timeout,a.output_lines])<=0:raise ValueError('Positive lengths, concurrencies, repeats, timeout and output-lines required')
    a.output.mkdir(parents=True,exist_ok=True)
    run=uuid.uuid4().hex
    metadata={'run_id':run,'utc':datetime.now(timezone.utc).isoformat(),'model':a.model,'tokenizer_revision':a.tokenizer_revision,'lengths':lengths,'incoming_concurrencies':cs,'repeats':a.repeats,'output_lines':a.output_lines,'template':json.loads(a.template_json),'source_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),'metrics':'Client estimates; server-native prefill/decode timing is separate.'}
    ...
    if not a.no_warmup:
        prompt,_=prepare_prompt(tok,1024,uuid.uuid4().hex,a.output_lines)
    ...
    for length in lengths:
        for concurrency in cs:
            for repeat in range(a.repeats):
                prepared=[prepare_prompt(tok,length,uuid.uuid4().hex,a.output_lines) for _ in range(concurrency)]
