...
def _get_kv_cache_groups_glm5_next(
    ...
    if draft_specs:
        ...
        if (
            ...
            # page. DFlash arrives with a 16-token kernel block, but charging
            # its 2048-token sliding window at that granularity consumes 385
            # global block ids. Every id carries the complete GLM-5-Next
            # shared-pool byte cost, inflating one-request KV admission from
            # 8.14 GiB to 22.68 GiB. Use a 512-token manager block, split into supported
            # 64-token kernel blocks. It tiles both the source draft block
            # and the target manager block, leaves the scheduler LCM
            # unchanged, and retains the ordinary contiguous draft tensors.
            if any_draft.block_size < 512:
                draft_block = 512
                assert draft_block % any_draft.block_size == 0, (
                    "draft source block must tile the promoted manager block"
                )
                assert mla_block % draft_block == 0, (
                    "promoted draft block must tile the target manager block"
                )
                assert any_draft.sliding_window % draft_block == 0, (
                    "promoted draft block must tile its sliding window"
                )
                new_draft_specs = {
                    name: replace(s, block_size=draft_block)
                    for name, s in draft_specs.items()
                }
            else:
