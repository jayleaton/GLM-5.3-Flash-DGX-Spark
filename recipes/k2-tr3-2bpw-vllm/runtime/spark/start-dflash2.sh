...
DFLASH_ROOT="${GLM53_DFLASH_ROOT:?set GLM53_DFLASH_ROOT to the pinned DFlash2 snapshot}"
IMAGE="${GLM53_IMAGE:?set GLM53_IMAGE to the immutable accepted DFlash image digest}"
CACHE_ROOT="${GLM53_CACHE_ROOT:-$PWD/.cache/vllm-glm53-exact2-dflash}"
SERVED_MODEL_NAME="${GLM53_SERVED_MODEL_NAME:-glm-5.3-flash-exl3-k2-dflash}"
HOST="${GLM53_HOST:-127.0.0.1}"
PORT="${GLM53_PORT:-18080}"

python3 - "$MODEL_ROOT" "$DFLASH_ROOT" <<'PY'
import json
import sys
...
assembly = json.loads((model / "assembly-status.json").read_text())
config = json.loads((model / "config.json").read_text())["quantization_config"]
index = json.loads((model / "model.safetensors.index.json").read_text())
references = set(index["weight_map"].values())
root_shards = {path.name for path in model.glob("*.safetensors")}
...
exec docker run --rm \
  --name glm53-flash-exl3-k2-dflash \
  --gpus all \
  --ipc host \
  --network host \
  -v "$MODEL_ROOT":/model:ro \
  -v "$DFLASH_ROOT":/dflash:ro \
  ...
  --compilation-config '{"cudagraph_mode":"FULL_DECODE_ONLY","custom_ops":["all"],"cudagraph_capture_sizes":[8],"max_cudagraph_capture_size":8}' \
